...
void LaptopFactory::SetFactoryId(int id)		{	node_id = id; }
...
void LaptopFactory::Handle_Replication_Request(ReplicationRequest * rep_req){
	...
	leader_id = rep_req -> Get_node_id();
	last_log_index = rep_req->Get_last_log_index();
...
EngineerThread(std::unique_ptr<ServerSocket> socket, int id) {
	...
	else if (connection_descriptor == PFA_CONNECTED){		//connection coming from the leader
...
void LaptopFactory::AdminThread() {
	...
	while (true) {
		...
		if (committed_index == last_log_index + 1){
			committed_index++;
			Apply_Committed_Op();
		}
		...
		ReplicationRequest rep_req;
		rep_req.Set_Rep_Req(node_id, committed_index, last_log_index, opcode, arg1, arg2);
		...
		committed_index = last_log_index;
		Apply_Committed_Op();

		req->laptop.SetAdminId(leader_id);
